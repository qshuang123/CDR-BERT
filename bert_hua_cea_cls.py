...
logging.basicConfig(level=logging.INFO, filename='bert_hua_cea_cls.log')
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f > 0.6:
            save_file("bert_hua_cea_cls_" + str(f) + "_predict", pred_test_y)
            model.save('./models/bert_hua_cea_cls_' + str(f) + '.h5')


def bert_gea_model(drop, k):
    ...
    res = concatenate([CLS_encode, context1_rep,context2_rep,entity1_vector,entity2_vector])
    res = Dense(1024)(res)
    res = Dense(256)(res)
    res = Dense(64)(res)
