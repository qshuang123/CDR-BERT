...
logging.basicConfig(level=logging.INFO, filename='bert_hua_entity.log')
...
def entity_extract(input):
    ...
    result = pos*input[0]
    result1 = GlobalMaxPooling1D()(result)
    return result1
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f > 0.6:
            save_file("bert_hua_entity_" + str(f) + "_predict", pred_test_y)
            model.save('./models/bert_hua_entity_' + str(f) + '.h5')


def bert_entity_model(drop,k):
    ...
    res = concatenate([CLS_encode, entity1_encodes, entity2_encodes])
    res = Dense(1024)(res)
    res = Dense(256)(res)
    res = Dense(64)(res)
...
if __name__ == "__main__":
    ...
    k = 400
    drop = 0.3
    model = bert_entity_model(drop, k)
