...
class Reader:
                ...
                for j in range(i+1,len(key_list)):
                    ...
                    if((e1[3]=='Chemical'and e2[3]=='Disease')or(e2[3]=='Chemical'and e1[3]=='Disease')):
                        ...
                        if(e1[3]=='Chemical'):
                            e1_type = 'Ch_'
                            e2_type = 'Ds_'
                            e1_tagged_start='Ch_start'
                            e1_tagged_end='ch_end'
                            e2_tagged_start='Ds_start'
                            e2_tagged_end='ds_end '
                        else:
                            e2_type = 'Ch_'
                            e1_type = 'Ds_'
                            e2_tagged_start = 'Ch_start'
                            e2_tagged_end = 'ch_end'
                            e1_tagged_start = 'Ds_start'
                        ...
                        replace_ins=all_text.replace(e1_type+e1_id,replace_e1)
                        replace_ins=replace_ins.replace(e2_type+e2_id,replace_e2)
                        train_instances.append(replace_ins)
                        if len(train_instances)==48:
                            print()
    ...
    def divide_sentence(self,train_instances):
        ...
        for doc in train_instances:
            ...
            while(index!=-1):
                count+=1
                if(index < len(doc)-2 and doc[index+1] == ' ' and doc[index+2].isupper() and index-start>20):
    ...
    def tokenizer(self,train_all_doc,dev_all_doc,test_all_doc,maxsentence,mylen):
        ...
        for i in all_document[0]:
            ...
            train_index_x = train_index_x[:min(maxsentence,len(train_index_x))]
    ...
    def delete_sentence(self,train):
        """
        save the sentences between target entity
        :param train:
        :return:
        """
        for i in range(len(train)):
            ...
            for j in range(len(train[i])):
                index1 = train[i][j].find("Ch_start")
                index2 = train[i][j].find("Ds_start")
            ...
            for k in range(len(train[i])):
                p=len(train[i])-k-1
                index1 = train[i][p].find("Ch_start")
                index2 = train[i][p].find("Ds_start")
                if index1!=-1 or index2!=-1:
                    end = p
                    break
            ...
            if start!=-1 and end !=-1:
                temp = []
                for q in range(len(train[i])):
                    if q >= start and q <= end:
                        temp.append(train[i][q])
                train[i]=temp
        ...
        for i in range(len(train)):
            for j in range(len(train[i])):
                while train[i][j].find("Ch_start")!=-1 :
                    index1 = train[i][j].find("Ch_start")
                    index2 = train[i][j].find("ch_end")
                    if index1!=-1 and index2!=-1:
                        train[i][j]=train[i][j][0:index1]+"Chemical"+train[i][j][index2+6:]
                        train[i][j]=train[i][j].lstrip(" ")
                        train[i][j] = train[i][j].replace("  ", " ")
                while train[i][j].find("Ds_start")!=-1:
                    index1 = train[i][j].find("Ds_start")
                    index2 = train[i][j].find("ds_end")
                    if index1 != -1 and index2 != -1:
                        train[i][j] = train[i][j][0:index1] + "Disease" + train[i][j][index2 + 6:]
                        train[i][j] = train[i][j].lstrip(" ")
                        train[i][j] = train[i][j].replace("  "," ")
                while train[i][j].find("Ch_")!=-1:
                    index1 = train[i][j].find("Ch_")
                    if index1 != -1:
                        train[i][j] = train[i][j][0:index1] + "Entity" + train[i][j][index1+10:]
                        train[i][j] = train[i][j].lstrip(" ")
                        train[i][j] = train[i][j].replace("  ", " ")
                while train[i][j].find("Ds_")!=-1:
                    index1 = train[i][j].find("Ds_")
                    if index1 != -1:
                        train[i][j] = train[i][j][0:index1] + "Entity" + train[i][j][index1+10:]
                        train[i][j] = train[i][j].lstrip(" ")
                        train[i][j] = train[i][j].replace("  ", " ")
...
if __name__ == "__main__":
    ...
    print()
     ...
     #
     # para_comput(train_all_doc, 15, 50)
     # para_comput(dev_all_doc, 15, 50)
     # para_comput(test_all_doc, 15, 50)
     #
     # maxsentence = 15
     # maxlen = 50
     #
     ...
     #
     # myreader.save_file("train_sentence",train_all_doc)
     # myreader.save_file("dev_sentence", dev_all_doc)
     # myreader.save_file("test_sentence", test_all_doc)
