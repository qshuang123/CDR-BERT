...
def entity_extract(input):
    ...
    pos = K.repeat_elements(pos, lstm_out_shape, -1)
    result = pos*input[0]
    return result
...
def bert_cea_model(drop):
    ...
    context_bert = bert_model([inputs_indices, inputs_segment])
    for l in bert_model.layers:
        l.trainable = True

    context_vector = keras.layers.Reshape((-1,4,max_sentence_length/4,bert_out_shape))(context_bert)
    context_lstm = TimeDistributed(Bidirectional(GRU(units=lstm_out_shape/4,dropout=drop,return_sequences=True)))(context_vector)
    lstm_vector = keras.layers.Reshape((-1, max_sentence_length, lstm_out_shape))(context_lstm)

    entity1_encodes = Lambda(entity_extract, output_shape=entity_extract_output_shape)([lstm_vector, entity1_mask])
    entity2_encodes = Lambda(entity_extract, output_shape=entity_extract_output_shape)([lstm_vector, entity2_mask])
