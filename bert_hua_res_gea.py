...
logging.basicConfig(level=logging.INFO, filename='bert_hua_res_gea.log')
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f > 0.6:
            save_file("bert_hua_res_gea_" + str(f) + "_predict", pred_test_y)
            model.save('./models/bert_hua_res_gea_' + str(f) + '.h5')
...
def bert_gea_model(drop):
    ...
    entity1 = keras.layers.Add()([entity1_encodes, entity1_atten])
    entity2 = keras.layers.Add()([entity2_encodes, entity2_atten])
    LayerNormalization()

    entity1_vector = GlobalMaxPooling1D()(entity1)
    entity2_vector = GlobalMaxPooling1D()(entity2)

    res = concatenate([entity1_vector, entity2_vector])
    ...
    model.summary()
    model.compile(optimizer='rmsprop', loss=[binary_focal_loss(alpha=0.82, gamma=2)], metrics=['accuracy'])
