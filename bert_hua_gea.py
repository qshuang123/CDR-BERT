...
logging.basicConfig(level=logging.INFO, filename='bert_hua_gea.log')
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f > 0.6:
            save_file("bert_hua_gea_" + str(f) + "_predict", pred_test_y)
            model.save('./models/bert_hua_gea_' + str(f) + '.h5')
