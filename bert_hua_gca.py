...
logging.basicConfig(level=logging.INFO, filename='bert_hua_gca.log')
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f > 0.6:
            save_file("bert_hua_gca_" + str(f) + "_predict", pred_test_y)
            model.save('./models/bert_hua_gca_' + str(f) + '.h5')
...
def bert_gca_model(drop,k):
    ...
    # input_mask = Lambda(lambda x: K.cast(K.greater(x, 0), 'float32'))(inputs_indices)
    ...
    res = concatenate([context1_rep, context2_rep])
